...
func (suite *baseTestSuite) TestDocument() {
	...
	suite.NoError(err)
	suite.Empty(documents)
	err = suite.AddScores(ctx, "visibility", "", []Score{{Id: "item", Score: 2, Timestamp: ts}})
	suite.NoError(err)
	err = suite.UpdateScores(ctx, []string{"visibility"}, new(""), "item", ScorePatch{IsHidden: new(true)})
	suite.NoError(err)
	err = suite.AddScores(ctx, "visibility", "", []Score{{Id: "item", Score: 20, Timestamp: ts.Add(time.Millisecond)}})
	suite.NoError(err)
	documents, err = suite.GetScores(ctx, "visibility", "", []string{"item"})
	suite.NoError(err)
	suite.Equal([]Score{{Id: "item", Score: 20, Timestamp: ts.Add(time.Millisecond)}}, documents)
